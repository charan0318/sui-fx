/**
 * SUI-FX Original Validation Schemas
 * Enhanced input validation for SUI testnet token drops
 */

import Joi from 'joi';
import { config } from '../config/index.js';

// Enhanced Sui address validation with checksum support
const SUI_ADDRESS_REGEX = /^(0x)?[0-9a-fA-F]{64}$/;

// [validation] Advanced Sui address validator with enhanced checks
...
// [validation] SUI drop request schema - original implementation
export const dropRequestSchema = Joi.object({
  // Primary address field with enhanced validation
  address: Joi.string()
    .optional()
    .custom(suiAddressValidator, 'Enhanced Sui address validation')
    .messages({
      'string.empty': 'Wallet address is required for SUI token drop',
      'any.invalid': 'Invalid Sui wallet address format. Must be 64 hex characters with optional 0x prefix',
    }),
