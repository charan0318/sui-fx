...
export default function Faucet() {
  ...
  // Get rate limit info for current wallet (TODO: implement endpoint)
  const walletAddress = form.watch("walletAddress");
  const rateLimitInfo = null; // Temporarily disabled until we implement the endpoint

  const requestTokensMutation = useMutation({
    mutationFn: async (data: FaucetRequestForm) => {
      // Backend expects 'walletAddress' field
      const requestData = { walletAddress: data.walletAddress };
      return apiRequest("POST", "/faucet/request", requestData);
    },
    onSuccess: (response) => {
      console.log("Faucet success response:", response);
      setLastTransaction({ 
        success: true, 
        data: response, 
        transactionHash: response.transactionHash,
        message: response.message || "Tokens sent successfully!",
        explorerUrl: response.explorerUrl || `https://suiscan.xyz/testnet/tx/${response.transactionHash}`
      });
      form.reset();
      toast({
        title: "Transaction Successful!",
        description: response.message || "SUI tokens have been sent to your wallet.",
      });
                                  ...
                                  <div className="relative">
                                    ...
                                    {field.value && (
                                      <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                                        ...
                                        ) : /^0x[a-fA-F0-9]{64}$/.test(field.value) ? (
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => copyToClipboard(lastTransaction.transactionHash)}
              ...
              >
                ...
                <Card className="bg-black/30 border-gray-700/50 backdrop-blur-xl hover:bg-black/40 transition-all duration-300 relative z-10">
                  ...
                  <CardContent>
                    ...
                    {/* Rate limit info - TODO: implement endpoint
                    {rateLimitInfo && (
